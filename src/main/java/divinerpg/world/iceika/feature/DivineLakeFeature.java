...
public class DivineLakeFeature extends Feature<LakeConfig> {
    private final BlockState AIR;

    public DivineLakeFeature() {
        super(opts -> new LakeConfig(opts));

        AIR = Blocks.CAVE_AIR.getDefaultState();
    }

    @Override
    public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, LakeConfig config) {
        ...
        if (pos.getY() <= 4) {
            ...
            if (!worldIn.getChunk(chunkpos.x, chunkpos.z, ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences(Feature.VILLAGE.getStructureName()).isEmpty()) {
                    ...
                    for(int l2 = 0; l2 < 16; ++l2) {
                        for(int k = 0; k < 8; ++k) {
                            ...
                            if (flag) {
                                ...
                                if (k < 4 && !material.isSolid() && worldIn.getBlockState(pos.add(k1, k, l2)) != config.liquid) {
                                    return false;
                                }
                            }
                        }
                    ...
                    for(int i3 = 0; i3 < 16; ++i3) {
                        for(int i4 = 0; i4 < 8; ++i4) {
                            if (aboolean[(l1 * 16 + i3) * 8 + i4]) {
                                worldIn.setBlockState(pos.add(l1, i4, i3), i4 >= 4 ? AIR : config.liquid, 2);
                            }
                        }
                    }
                }
                ...
                if (config.liquid.getMaterial() == Material.LAVA) {
                        ...
                        for(int k3 = 0; k3 < 16; ++k3) {
                            for(int k4 = 0; k4 < 8; ++k4) {
                                ...
                                if (flag1 && (k4 < 4 || rand.nextInt(2) != 0) && worldIn.getBlockState(pos.add(j2, k4, k3)).getMaterial().isSolid()) {
                                    worldIn.setBlockState(pos.add(j2, k4, k3), config.topBlock, 2);
                                }
                            }
                        }
                    }
                ...
                if (config.liquid.getMaterial() == Material.WATER) {
