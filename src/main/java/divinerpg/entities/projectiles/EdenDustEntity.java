...
public class EdenDustEntity extends ProjectileItemEntity {
    ////////////////////
    // OVERRIDE THESE VALUES
    // damage, render, getEntityType()
    ///////////////////
    private static LazyLoadBase<Item> render = new LazyLoadBase<>(() -> ItemRegistry.edenDust);
    private final int damage = 10;
    private final IParticleData particle = new ItemParticleData(ParticleTypes.ITEM, new ItemStack(render.getValue()));


    protected EdenDustEntity(World world) {
        super(getEntityType(), world);
    }

    public EdenDustEntity(EntityType<? extends ProjectileItemEntity> type, World worldIn) {
        super(type, worldIn);
    }

    public EdenDustEntity(World world, LivingEntity thrower) {
        super(getEntityType(), thrower, world);
    }

    private static EntityType<? extends ProjectileItemEntity> getEntityType() {
        return EntitiesRegistry.eden_dust;
    }
